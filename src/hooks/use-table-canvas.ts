...
export function useTableCanvas(options: UseTableCanvasOptions = {}) {
  ...
  const [tables, setTables] = useState<PlacedTable[]>(initialTables)
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const rawX = e.clientX - rect.left - tableSize / 2
      const rawY = e.clientY - rect.top - tableSize / 2

      const x = Math.max(16, Math.min(rect.width - tableSize - 16, rawX))
      const y = Math.max(28, Math.min(rect.height - tableSize - 36, rawY))
  ...
  const handleQuickAdd = useCallback(
    (templateType: string = 'table1') => {
      ...
      const x = Math.max(20, Math.min(rect.width - tableSize - 20, 40 + offset))
      const y = Math.max(32, Math.min(rect.height - tableSize - 40, 40 + offset))
      ...
      if (dragRafRef.current === null) {
        dragRafRef.current = requestAnimationFrame(() => {
          ...
          setTables((prev) => {
            ...
            const x = Math.max(12, Math.min(canvasWidth - curSize - 12, update.x))
            const y = Math.max(28, Math.min(canvasHeight - curSize - 36, update.y))
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (pendingDragRef.current && currentRect) {
        const update = pendingDragRef.current
